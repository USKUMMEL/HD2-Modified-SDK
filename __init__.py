...
from . import ParticleModder as particle_modder_m
...
importlib.reload(particle_modder_m)
...
def FindEntryArchiveMatches(FileID, TypeID):
    matches = []
    file_id = int(FileID)
    type_id = int(TypeID)

    if Global_TocManager.ActivePatch and Global_TocManager.ActivePatch.GetEntry(file_id, type_id):
        patch_name = Global_TocManager.ActivePatch.LocalName if Global_TocManager.ActivePatch.LocalName != "" else Global_TocManager.ActivePatch.Name
        matches.append(("patch", patch_name))

    for archive in Global_TocManager.LoadedArchives:
        if archive.GetEntry(file_id, type_id):
            matches.append(("archive", archive.Name))

    for archive in Global_TocManager.SearchArchives:
        if archive.HasEntry(file_id, type_id):
            matches.append(("search", archive.Name))
...
def FormatArchiveMatch(match):
    match_type, archive_id = match
    if match_type == "patch":
        return f"Patch: {archive_id}"
    archive_name = GetArchiveNameFromID(str(archive_id))
    if archive_name != "":
        return f"{archive_id} ({archive_name})"
    if match_type == "search":
        return f"{archive_id} (search index)"
...
def ParseParticleVisualizerRefs(TocData):
    ...
    for system_index in range(num_particle_systems):
        ...
        if non_rendering != 0 or visualizer_offset == system_size:
            f.seek(system_offset + system_size)
            continue

        visualizer_pos = system_offset + visualizer_offset
        system_end = system_offset + system_size
        if visualizer_pos + 4 > len(f.Data) or system_end > len(f.Data):
            break

        f.seek(visualizer_pos)
        visualizer_type = f.uint32(0)
        ref = {
        ...
        if visualizer_type in [0, 3]:
            f.uint32(0)
            f.uint32(0)
            ref["material_id"] = f.uint64(0)
        elif visualizer_type == 2:
            ref["unit_id"] = f.uint64(0)
            ref["mesh_id"] = f.uint64(0)
            ref["material_id"] = f.uint64(0)
...
class ParticleSearchUsedIDsOperator(Operator):
    bl_label  = "Search Particle Used IDs"
    bl_idname = "helldiver2.particle_search_used_ids"
    bl_description = "Search material, texture and mesh IDs used by this particle and print archive locations to Blender Console"

    object_id: StringProperty()

    @staticmethod
    def _find_matches_in_scope(file_id, type_id, scoped_archives):
        matches = []
        fid = int(file_id)
        tid = int(type_id)

        if Global_TocManager.ActivePatch and Global_TocManager.ActivePatch.GetEntry(fid, tid):
            patch_name = Global_TocManager.ActivePatch.LocalName if Global_TocManager.ActivePatch.LocalName != "" else Global_TocManager.ActivePatch.Name
            matches.append(f"Patch: {patch_name}")

        for archive in scoped_archives:
            if archive.GetEntry(fid, tid):
                archive_name = GetArchiveNameFromID(str(archive.Name))
                if archive_name != "":
                    matches.append(f"{archive.Name} ({archive_name})")
                else:
                    matches.append(str(archive.Name))

        # Fallback 1: check other currently loaded archives
        if len(matches) == 0:
            for archive in Global_TocManager.LoadedArchives:
                if archive in scoped_archives:
                    continue
                if archive.GetEntry(fid, tid):
                    archive_name = GetArchiveNameFromID(str(archive.Name))
                    if archive_name != "":
        ...
        # Fallback 2: check indexed search archives (no archive load)
        if len(matches) == 0:
            indexed = []
            for archive in Global_TocManager.SearchArchives:
                if archive.HasEntry(fid, tid):
                    indexed.append(str(archive.Name))
            if len(indexed) > 0:
                indexed = sorted(set(indexed))
                archive_id = indexed[0]
                archive_name = GetArchiveNameFromID(archive_id)
                if archive_name != "":
                    matches.append(f"{archive_id} ({archive_name})")
                else:
                    matches.append(f"{archive_id} (indexed)")
    ...
    def execute(self, context):
        particle_ids = IDsFromString(self.object_id)
        if len(particle_ids) == 0:
            self.report({'ERROR'}, f"Invalid particle ID input: {self.object_id}")
            return {'CANCELLED'}

        for particle_id in particle_ids:
            particle_entry = Global_TocManager.GetEntry(particle_id, ParticleID, SearchAll=True, IgnorePatch=False)
            if particle_entry is None:
                PrettyPrint(f"Could not find particle ID: {particle_id}", "warn")
                continue

            try:
                refs = ParseParticleVisualizerRefs(particle_entry.TocData)
            except Exception as ex:
                PrettyPrint(f"Failed to parse particle ID: {particle_id} ({ex})", "warn")
                continue

            scoped_archives = [archive for archive in Global_TocManager.LoadedArchives if archive.GetEntry(particle_id, ParticleID)]
            if len(scoped_archives) == 0 and Global_TocManager.ActiveArchive is not None:
                scoped_archives = [Global_TocManager.ActiveArchive]

            material_ids = sorted({int(ref["material_id"]) for ref in refs if ref["material_id"] is not None})
            unit_ids = sorted({int(ref["unit_id"]) for ref in refs if ref["unit_id"] is not None})
            mesh_ids = sorted({int(ref["mesh_id"]) for ref in refs if ref["mesh_id"] is not None})
            texture_ids = set()
            mesh_internal_to_units = {}

            for material_id in material_ids:
                material_entry = Global_TocManager.GetEntry(material_id, MaterialID, SearchAll=False, IgnorePatch=False)
                if material_entry is None:
                    continue
                if not material_entry.IsLoaded:
                    material_entry.Load(False, False)
                for tex_id in material_entry.LoadedData.TexIDs:
                    texture_ids.add(int(tex_id))

            for ref in refs:
                if ref["mesh_id"] is None:
                    continue
                mesh_id = int(ref["mesh_id"])
                unit_id = int(ref["unit_id"]) if ref["unit_id"] is not None else None
                if mesh_id not in mesh_internal_to_units:
                    mesh_internal_to_units[mesh_id] = set()
                if unit_id is not None:
                    mesh_internal_to_units[mesh_id].add(unit_id)

            PrettyPrint(f"========== Particle {particle_id} ==========")
            PrettyPrint("Materials:")
            if len(material_ids) == 0:
                PrettyPrint("  (none)")
            for material_id in material_ids:
                archives = self._find_matches_in_scope(material_id, MaterialID, scoped_archives)
                archive_text = ", ".join(archives) if archives else "archive unknown"
                PrettyPrint(f"  {material_id} [{archive_text}]")

            PrettyPrint("Meshes (Unit IDs):")
            if len(unit_ids) == 0:
                PrettyPrint("  (none)")
            for unit_id in unit_ids:
                archives = self._find_matches_in_scope(unit_id, UnitID, scoped_archives)
                if not archives:
                    archives = self._find_matches_in_scope(unit_id, CompositeUnitID, scoped_archives)
                archive_text = ", ".join(archives) if archives else "archive unknown"
                PrettyPrint(f"  {unit_id} [{archive_text}]")

            PrettyPrint("Meshes (Internal IDs):")
            if len(mesh_ids) == 0:
                PrettyPrint("  (none)")
            for mesh_id in mesh_ids:
                if mesh_id <= 0xFFFFFFFF:
                    owner_units = sorted(mesh_internal_to_units.get(mesh_id, []))
                    resolved_units = []
                    for owner_unit_id in owner_units:
                        unit_entry = Global_TocManager.GetEntry(owner_unit_id, UnitID, SearchAll=False, IgnorePatch=False)
                        if unit_entry is None:
                            continue
                        # Keep this search non-invasive: do not auto-load unit data here,
                        # because unit loading is noisy and may trigger unrelated console logs.
                        if not unit_entry.IsLoaded:
                            continue
                        if hasattr(unit_entry.LoadedData, "RawMeshes"):
                            for unit_mesh in unit_entry.LoadedData.RawMeshes:
                                if int(unit_mesh.MeshID) == mesh_id:
                                    resolved_units.append(owner_unit_id)
                                    break
                    if len(resolved_units) > 0:
                        resolved_text = ", ".join([str(x) for x in sorted(set(resolved_units))])
                        PrettyPrint(f"  {mesh_id} [internal mesh hash, resolved in unit: {resolved_text}]")
                    elif len(owner_units) > 0:
                        owner_text = ", ".join([str(x) for x in owner_units])
                        PrettyPrint(f"  {mesh_id} [internal mesh hash, owner unit: {owner_text}]")
                    else:
                        PrettyPrint(f"  {mesh_id} [internal mesh hash]")
                else:
                    archives = self._find_matches_in_scope(mesh_id, UnitID, scoped_archives)
                    if not archives:
                        archives = self._find_matches_in_scope(mesh_id, CompositeUnitID, scoped_archives)
                    archive_text = ", ".join(archives) if archives else "archive unknown"
                    PrettyPrint(f"  {mesh_id} [{archive_text}]")

            sorted_texture_ids = sorted(texture_ids)
            PrettyPrint("Textures:")
            if len(sorted_texture_ids) == 0:
                PrettyPrint("  (none)")
            for texture_id in sorted_texture_ids:
                archives = self._find_matches_in_scope(texture_id, TexID, scoped_archives)
                archive_text = ", ".join(archives) if archives else "archive unknown"
                PrettyPrint(f"  {texture_id} [{archive_text}]")

        self.report({'INFO'}, "Particle dependency search complete. See Blender Console for details.")
        return {'FINISHED'}
...
class WM_MT_button_context(Menu):
    ...
    def draw_entry_buttons(row, Entry):
        ...
        if AreAllUnits:
            ...
            row.operator("helldiver2.particle_search_used_ids", icon='VIEWZOOM', text="Search Particle Dependencies").object_id = FileIDStr
        #elif AreAllParticles:
